...
// +kubebuilder:webhook:path=/validate-workspace-jupyter-org-v1alpha1-workspaceintegrationtemplate,mutating=false,failurePolicy=fail,sideEffects=None,groups=workspace.jupyter.org,resources=workspaceintegrationtemplates,verbs=create;update,versions=v1alpha1,name=vworkspaceintegrationtemplate-v1alpha1.kb.io,admissionReviewVersions=v1,serviceName=jupyter-k8s-controller-manager,servicePort=9443

// WorkspaceIntegrationTemplateCustomValidator statically validates a WorkspaceIntegrationTemplate on
// create and update. failurePolicy=fail: a malformed template is rejected outright, because it can
// only ever produce broken integrations.
//
