...
// It gates two update-path checks that must run only when the user actually changed the refs, mirroring
// how template validation gates on specChanged:
...
//   - per-resource authorization (ValidateGetResourceRefs) -- the objects a workspace authorizes are a
//     function of its refs (template + parameters) and its namespace, and the namespace is immutable, so
//     unchanged refs mean the user already passed authorization when the refs were introduced.
//
// Uses semantic equality (nil slice == empty slice) like the sibling comparators, so a no-op round-trip
// does not force a re-check.
