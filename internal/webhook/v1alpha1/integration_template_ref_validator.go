...
//
//   - Validate: correctness checks that apply to EVERY caller -- namespace scope, template existence, and
//     parameter completeness. The webhook runs it before the controller/admin bypass, so a dangling
//     reference or a missing parameter is reported even for privileged writers (it is a user error, not a
//     permission concern).
...
// authorization pass (ValidateGetResourceRefs) reuses them instead of re-resolving: correctness always
// runs first and for every caller, so the templates it loads are exactly the ones authorization needs.
// On error the returned slice is nil (the webhook rejects before authorizing).
...
// reusing them avoids re-resolving each template a second time in the same admission request. Validate
// always runs first (for every caller, before the admin bypass), so the templates are already loaded.
