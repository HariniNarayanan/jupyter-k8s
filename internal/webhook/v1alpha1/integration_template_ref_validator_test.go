...
var _ = Describe("IntegrationTemplateRefValidator namespace scope", func() {
	...
	It("allows a by-name ref (empty namespace) that resolves only in the shared namespace", func() {
		// The template lives ONLY in the shared namespace, and the ref omits its namespace. The
		// validator must fall back from the workspace's own namespace to the shared namespace --
		// mirroring the controller's getIntegrationTemplate resolution -- rather than rejecting. This
		// is the chart-install path: an admin ships the template into the shared namespace once and
	...
	It("rejects a by-name ref when the template is absent from BOTH the own and shared namespaces", func() {
