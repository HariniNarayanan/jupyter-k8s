...
// getIntegrationTemplate loads the template with the WorkspaceTemplate resolver's namespace fallback:
// ref.namespace, else the workspace namespace, else the shared namespace (defaultNamespace, if set).
// Missing after all steps is a fail-closed error.
func getIntegrationTemplate(
	...
	err := c.Get(ctx, client.ObjectKey{Name: ref.Name, Namespace: ns}, template)

	// Fall back to the shared namespace when the template is not in the primary namespace.
	if apierrors.IsNotFound(err) && defaultNamespace != "" && ns != defaultNamespace {
		fallbackErr := c.Get(ctx, client.ObjectKey{Name: ref.Name, Namespace: defaultNamespace}, template)
		if fallbackErr == nil {
			return template, nil
		}
		if !apierrors.IsNotFound(fallbackErr) {
			// A non-NotFound fallback error (e.g. forbidden) is a real failure -- surface it, don't mask as not-found.
			return nil, fmt.Errorf("failed to get integration template %q from shared namespace %q: %w", ref.Name, defaultNamespace, fallbackErr)
		}
		return nil, fmt.Errorf("integration template %q not found in namespace %q or shared namespace %q", ref.Name, ns, defaultNamespace)
	}
