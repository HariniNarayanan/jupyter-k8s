...
type StateMachine struct {
	...
	// is then skipped and integrations report no status.
...
func (sm *StateMachine) probeIntegrationStatus(
	...
	var shortestProbeInterval time.Duration
	// appendStatus preserves the prior timestamp on an unchanged verdict, records the status, and folds
	// its (backoff-aware) re-probe interval into the shortest interval across all integrations. Every
	// integration re-probes on the same operator base cadence (DefaultIntegrationProbePeriod).
	appendStatus := func(s workspacev1alpha1.IntegrationStatus) {
		preserveConditionTimestamp(prior[s.Name], &s)
		statuses = append(statuses, s)
		shortestProbeInterval = getShorterInterval(shortestProbeInterval,
			integrationProbeRequeueAfter(&s, now))
	}
