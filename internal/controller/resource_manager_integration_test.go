	...
	return &workspacev1alpha1.WorkspaceIntegrationTemplate{
		...
		Spec: workspacev1alpha1.WorkspaceIntegrationTemplateSpec{
			...
			ResourceRefs: []workspacev1alpha1.ResourceRef{{
				Name: "rayCluster", APIVersion: rayAPIVersion, Kind: rayClusterKind,
...
// freezeResourceManagerWithSharedNS builds a ResourceManager whose integration resolver has a
// shared-namespace fallback configured (via WorkspaceControllerOptions.DefaultTemplateNamespace).
func freezeResourceManagerWithSharedNS(c client.Client, sharedNS string) *ResourceManager {
	opts := WorkspaceControllerOptions{
		ApplicationImagesPullPolicy: corev1.PullIfNotPresent,
		DefaultTemplateNamespace:    sharedNS,
	}
	return NewResourceManager(
		c, c.Scheme(),
		NewDeploymentBuilder(c.Scheme(), opts, c),
...
// TestIntegrationFreeze_SharedNamespaceTemplateFallback verifies that a WorkspaceIntegrationTemplate
// published only in the shared namespace is resolved for a workspace in a different namespace that
// references it by name alone (no integrationTemplateRefs[].namespace) -- mirroring the
// WorkspaceTemplate shared-namespace fallback.
func TestIntegrationFreeze_SharedNamespaceTemplateFallback(t *testing.T) {
	ctx := context.Background()
	c, stop := startFreezeEnvtest(t)
	defer stop()
	...
	rm := freezeResourceManagerWithSharedNS(c, sharedNS)
	deployName := GenerateDeploymentName("shared-ws")

	// Workspace in the default namespace references the template by NAME ONLY (no namespace) -- so it
	// is not in the workspace ns and can only be found via the shared-namespace fallback.
	...
	if !hasSidecar(d) || !filepathContainsStr(sidecarArgsOf(d), "svc-shared:6379") {
		t.Fatalf("SHARED-NS FALLBACK: expected sidecar resolved from the shared-namespace template targeting svc-shared:6379, got args=%q", sidecarArgsOf(d))
	}
	if fr := findResolvedIntegration(&ws.Status, rayIntegrationName); fr == nil {
		t.Fatalf("SHARED-NS FALLBACK: expected a frozen record from the shared-namespace template")
	}
	t.Logf("SHARED-NS FALLBACK PASS: template in %q resolved for workspace in %q by name alone; sidecar=%q", sharedNS, wsNS, sidecarArgsOf(d))
}
