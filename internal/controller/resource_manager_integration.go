...
// This file is the capture side of integrations: it reads each integration's referenced resource (e.g.
// a RayCluster) and freezes the resolved substitutions into status.resolvedIntegrations -- the ONLY
// place that reads a referenced resource. The build side renders the pod from those frozen values.
