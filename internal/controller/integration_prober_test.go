...
func TestIntegrationProber_Verdicts(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			prober := NewIntegrationProber(fake.NewClientBuilder().WithScheme(scheme.Scheme).Build(), tc.exec)
...
func TestIntegrationProber_FindRunningPod(t *testing.T) {
	...
	t.Run("running pod found", func(t *testing.T) {
		p := NewIntegrationProber(fake.NewClientBuilder().WithScheme(scheme.Scheme).WithObjects(runningWSPod(ns, ws)).Build(), &fakeExec{})
	...
	t.Run("no pod -> error", func(t *testing.T) {
		p := NewIntegrationProber(fake.NewClientBuilder().WithScheme(scheme.Scheme).Build(), &fakeExec{})
		if _, err := p.FindRunningPod(context.Background(), workspace); err == nil {
	...
	t.Run("terminating pod skipped -> error", func(t *testing.T) {
		...
		p := NewIntegrationProber(fake.NewClientBuilder().WithScheme(scheme.Scheme).WithObjects(term).Build(), &fakeExec{})
		if _, err := p.FindRunningPod(context.Background(), workspace); err == nil {
...
func TestIntegrationProbeRequeueAfter(t *testing.T) {
	// base is the operator cadence the function uses internally (DefaultIntegrationProbePeriod); the test
	// asserts against that same constant.
	base := DefaultIntegrationProbePeriod
	now := time.Now()
	ready := &workspacev1alpha1.IntegrationStatus{State: IntegrationStateReady}

	// Ready -> base period, no backoff.
	if got := integrationProbeRequeueAfter(ready, now); got != base {
		t.Errorf("ready: want base %v, got %v", base, got)
	}

	// Failing: backoff grows with how long it has been failing (LastTransitionTime is held stable across
	// a streak by preserveConditionTimestamp), capped at MaxIntegrationProbeBackoff.
	failingFor := func(d time.Duration) *workspacev1alpha1.IntegrationStatus {
		return &workspacev1alpha1.IntegrationStatus{
			State: IntegrationStateDegraded,
			Conditions: []metav1.Condition{{
				Type: IntegrationConditionTypeReady, Status: metav1.ConditionFalse,
				LastTransitionTime: metav1.NewTime(now.Add(-d)),
			}},
		}
	}
	if got := integrationProbeRequeueAfter(failingFor(0), now); got != base {
		t.Errorf("just-failed: want base %v, got %v", base, got)
	}
	if got := integrationProbeRequeueAfter(failingFor(base), now); got <= base {
		t.Errorf("failing 1 period: want backoff > base, got %v", got)
	}
	if got := integrationProbeRequeueAfter(failingFor(24*time.Hour), now); got != MaxIntegrationProbeBackoff {
		t.Errorf("failing long: want cap %v, got %v", MaxIntegrationProbeBackoff, got)
	}
}
...
func TestPreserveConditionTimestamp(t *testing.T) {
	...
	t.Run("same status+reason but different message -> prior timestamp kept (Message excluded)", func(t *testing.T) {
		...
		// stay stable so integrationProbeRequeueAfter's backoff clock keeps counting from the true onset.
